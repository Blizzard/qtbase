...
{
    bool isRealTopLevel = isTopLevel() && !isEmbedded();
    if (emptyDefaultMargins()) {
        RECT cr;
    ...
    return frameGeometry(handle(), isRealTopLevel);
}
...
QMargins QWindowsWindow::frameMargins() const
{
    ...
    if (testFlag(FrameDirty)) {
        ...
        m_data.frame = (m_data.flags & Qt::FramelessWindowHint || m_data.emptyDefaultMargins)
            ? QMargins(0, 0, 0, 0)
